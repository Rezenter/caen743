...
-  offset = число с плавающей точкой.
Хранит значение параметра *offset* из файла конфигурации.
-  triggerThreshold = число с плавающей точкой.
Хранит значение параметра *triggerThreshold* из файла конфигурации.
...
Сам сигнал в милливольтах.
Величина рассчитывается из разрядов АЦП по формуле:  
signal_mV = *offset* + signal_ADC * 2500 / 4096  
Неясным остаётся, почему разряды АЦП (signal_ADC) библиотека возвращает в виде числа с плавающей точкой.
Возможно, так применяются какие-то калибровки? 
...
- Почему разряды АЦП приходят в виде дробных чисел?
