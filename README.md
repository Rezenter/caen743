...
NIM, зелёный link, pll Lock на ведущей плате и clk in на ведомых платах.
...
На текущий момент создан Python класс utils/chatter.py, реализующий протокол обмена сообщениями. 
Пример использования приведён в файле utils/control.py. 
...
Также реализован алгоритм для просмотра и стат-обработки полученных данных utils/viewer.py.
...
Время между соседними точками оцифровки вычисляется как 1/frequency и соответственно равно 0.3125, 0.625, 1.25 и 2.5 нс.
По умолчанию “3.2”.
...
Документация говорит, что 0000 => окно оцифровки от -2.5 до 0,
7FFF => окно оцифровки от -1.25 до 1.25, FFFF => окно оцифровки от 0 до 2.5.  
По умолчанию “6FFF”.
    - triggerThreshold = “HEX”, где HEX = [0, FFFF]
Число в шестнадцатеричной системе в строковом представлении.
Регистры 16-бит ЦАПа, которым задаётся уровень запуска по нулевому каналу.
Насколько я понимаю, 
выход ЦАП подаётся на компаратор, который сравнивает его с уровнем сигнала после подстройки постоянного смещения offset. 
Документация говорит, что 0000 = +1.25 V, 7FFF = 0 V, FFFF = -1.25 V.
По умолчанию “7FFF”.
- connection
...
При включении индикаторы на нём остаются в первоначальном состоянии: горит TTL и жёлтый индикатор передачи данных по 
оптоволокну. 
...
### main
[main.cpp](main.cpp) является точкой входа.


### Control
Клас описывается файлами [Control.cpp](Control.cpp) и [Control.h](Control.h).
